...
@Query(value="SELECT * FROM enitec.\"content\" c WHERE ct_code LIKE 'CT%'",nativeQuery = true)
	...
	@Query(value="select ct_code,ct_title,g_code,ct_info,ct_path_thumbnail,ct_star,ct_path,ct_release_Date,ct_modifydate from content where ct_title LIKE :keyword",nativeQuery=true)
